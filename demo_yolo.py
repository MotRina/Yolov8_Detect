...
TMP_DIR_PATH = "./PATH"
MODEL_DIR_PATH = "./PATH"
GENDER_MODEL_PATH = "./PATH/best.pt"
...
gender_model = YOLO(os.path.join(GENDER_MODEL_PATH))
...
def predict_gender(image):
    # Convert PIL image to cv2 format and resize
    image = np.array(image)
    image = cv2.resize(image, (224, 224))

    # Predict using YOLO gender classification model
    results = gender_model.predict(image)

    # Debug: Print results to understand its structure
    print(results)

    # Assuming results is a list of detections where each detection is a list or tuple
    # and the last element is the class ID (0 for 'female', 1 for 'male')
    # and the second last element is the confidence
    # Modify this according to the actual structure of results
    if len(results) > 0 and len(results[0]) > 1:
        gender = 'female' if results[0][-1] == 0 else 'male'
        confidence = results[0][-2]
    else:
        gender, confidence = 'unknown', 0
